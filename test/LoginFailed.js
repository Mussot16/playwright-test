...
(async () => {
  ...
  // Rellenar el formulario de login con credenciales no válidas
  await page.fill('#username', 'PapiMussot');
  await page.fill('#password', 'YepizSign');
 ...
 console.log('Esperando mensaje de error...');
  ...
  await page.waitForSelector('ul.errors li');

  // Verificar que el mensaje de error es el esperado
  const errorMessage = await page.textContent('ul.errors li');
  if (errorMessage.includes('Usuario o contraseña incorrectos')) {
    console.log('Login fallido: Credenciales no validas');
  } else {
